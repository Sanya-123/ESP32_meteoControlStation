...
void ST7735_Init() {
    ...
    ESP_LOGI("ST7735", "init");

    gpio_config_t io_conf;
    io_conf.intr_type = GPIO_INTR_DISABLE;//disable interrupt
    io_conf.mode = GPIO_MODE_OUTPUT;//set as output mode
    io_conf.pin_bit_mask = ((1ULL<<ST7735_DC_Pin) | (1ULL<<ST7735_CS_Pin));//bit mask of the pins that you want to set,e.g.GPIO18/19
...
//    gpio_set_direction(ST7735_CS_Pin, GPIO_MODE_OUTPUT);
    gpio_set_level(ST7735_CS_Pin, 1);
    spi_init();
    ESP_LOGI("ST7735", "OK init SPI");
//    PIN_CONFIGURATION(PIN_8);
//    PIN_CONFIGURATION(PIN_9);
//    //timer
...
//    TIM4->CNT = 0;
//    while( TimingDelay != 0 );
    vTaskDelay(delay_value / portTICK_RATE_MS);
}

//void TimingDelay_Decrement(void) {
//    if (TimingDelay != 0x00) TimingDelay--;

//    if (TimingDelay == 0x00) TIM4->CR1 &= ~TIM_CR1_CEN;//stop timer
//}
