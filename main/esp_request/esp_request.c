...
static int nossl_connect(request_t *req)
{
    ...
    REQ_CHECK(_socket < 0, "socket failed", return -1);
    ...
    ESP_LOGI(REQ_TAG, "[sock=%d],connecting to server IP:%s,Port:%s...",
             _socket, ipaddr_ntoa((const ip_addr_t*)&remote_ip.sin_addr.s_addr), (char*)port->value);
    if(connect(_socket, (struct sockaddr *)(&remote_ip), sizeof(struct sockaddr)) != 0) {
        close(_socket);
        return -1;
    }
    req->socket = _socket;
